...
class GATA(torch.nn.Module): ##### TransformerConv
    ...
    def forward(self, data):
        ...
        # print('kg_output_size',kg_output.size())
        ...
        #_________________________________________________
        # print('x',x.size())
        # print('xt_sum',xt_sum.size())
        xt_sum_ = xt_sum.view(1, 128)
        # print('xt_sum_',xt_sum.size())
        # concat
        xc = torch.cat((x, xt_sum_), 1)
...
def define_mol_encoder(is_freeze=True):
    ...
    mol_tokenizer = RobertaTokenizer.from_pretrained("model/seyonec_local/ChemBERTa-zinc-base-v1",
                                                     ...
                                                     truncation=True,
                                                     max_length=514)                                           
    mol_encoder = AutoModel.from_pretrained("model/seyonec_local/ChemBERTa-zinc-base-v1",
                                            local_files_only=True)
...
class DTI(nn.Module):
    ...
    def forward(self,data):
        ...
        SMILES_input = self.mol_tokenizer(SMILES, 
                                          return_tensors='pt',
                                          padding=True)
        
        #SMILES_input_size = len(SMILES_input)

        #print('SMILES_input_size',SMILES_input_size)
        ...
        pre_out = self.cls_out(x).squeeze(-1) # Pretrained out
