...
####
dataset = 'bindingdb'#[int(sys.argv[0])]
tasks= ['cold_all','cold_drug','cold_target']# cold_drug\cold_all
# Main program: iterate over different datasets
for task in tasks:
    print('\nrunning on ', wandb.config['model_st'] + '_' + dataset )
    processed_data_file_train = 'data_cold/'+ dataset+'/'+task+'/processed/processed_data_train.pt'
    processed_data_file_test = 'data_cold/' + dataset +'/'+task+'/processed/processed_data_test.pt'
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        print('please run create_data.py to prepare data in pytorch format!')
    else:
        train_data = GATADataset(root='data_cold/'+ dataset+'/'+task)
        test_data = GATADataset(root='data_cold/'+ dataset+'/'+task)
